...
process MUTECT2_CALLING {
    ...
    script:
    """
    ${params.database_path}/EVC_nextflow/gatk-4.6.0.0/gatk Mutect2 -independent-mates -R ${params.ref} -pon ${params.mutect2_pon} --germline-resource ${params.mutect2_germline} --native-pair-hmm-threads 2 --af-of-alleles-not-in-resource 0.00003125 --f1r2-tar-gz ${map.patient}_${map.tumor_meta.sample}_${chunk}-f1r2.tar.gz --normal-sample ${map.patient}_${map.normal_meta.sample}_normal --input ${map.normal} --tumor-sample ${map.patient}_${map.tumor_meta.sample}_tumor --input ${map.tumor} -L ${params.mutect2_interval_dir}/${chunk}-scattered.interval_list -O ${map.patient}_${map.tumor_meta.sample}_${chunk}-unfiltered.vcf
    """
}
