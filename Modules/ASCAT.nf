...
process ASCAT {
    ...
    output:
    tuple val(map), path("*alleleFrequencies*.txt"),      emit: allelefreq
    tuple val(map), path("*png"),                             emit: pngs
    tuple val(map), path("*BAF.txt"),                         emit: baf
    tuple val(map), path("*LogR.txt"),                        emit: logr
    tuple val(map), path("*raw.txt"),                        emit: cnvs
    tuple val(map), path("*segments.txt"),                    emit: segments
    tuple val(map), path("*cnvs.txt"),                        emit: cnv
    tuple val(map), path("*metrics.txt"),                     emit: metrics
    tuple val(map), path("purity_ploidy*.txt"),                emit: purityploidy
    ...
    write.table(ascat.output[["segments"]], file=paste0("${map.patient}_${map.sample}", ".segments.txt"), sep="\t", quote=F, row.names=F)
    QC = ascat.metrics(ascat.bc,ascat.output)
    write.table(QC, file=paste0("${map.patient}_${map.sample}", ".metrics.txt"), sep="\t", quote=F, row.names=F)

    # Save ASCAT objects
    save(ascat.bc, ascat.output, file = paste0("${map.patient}_${map.sample}", "_ASCAT_objects.Rdata"))

    cnvs=ascat.output[["segments"]][2:6]
    write.table(cnvs, file=paste0("${map.patient}_${map.sample}",".cnvs.txt"), sep="\t", quote=F, row.names=F, col.names=T)
    ...
    output_df <- data.frame(
        sample = "${map.patient}_${map.sample}",
        purity = purity,
        ploidy = ploidy
    )
